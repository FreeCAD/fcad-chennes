...
from addonmanager_addon_source_git import AddonDataSourceGit, GitAddonFactory
from AddonManagerTest.app.mocks import MockAddon, MockMetadata, AddonSignalCatcher, SignalCatcher
...
class TestAddonSourceGit(unittest.TestCase):
    def setUp(self):
        self.test_object = AddonDataSourceGit()
...
file://localhost/c:/users/test/FourthTest"""
        ...
        expected_result = [
            {
                "name": "Test",
                "url": "https://github.com/FreeCAD/Test",
                "branch": "main",
            },
            {
                "name": "AnotherTest",
                "url": "https://framagit.com/MyRepo/AnotherTest",
                "branch": "master",
            },
            {
                "name": "ThirdTest",
                "url": "https://salsa.debian.org/TestRepo/ThirdTest",
                "branch": "master",
            },
            {
                "name": "FourthTest",
                "url": "file://localhost/c:/users/test/FourthTest",
                "branch": "master",
            },
        ]
    ...
    def test_process_metadata_txt_line_normal(self):
        ...
        set_result = set()
        self.test_object._process_metadata_txt_line(test_data, set_result)
    ...
    def test_run_emits_finished(self):
        """Run emits finished()"""
        signal_catcher = SignalCatcher()
        self.test_object.finished.connect(signal_catcher.catch_signal)
        self.test_object.fetch_custom_repos = lambda:None
        self.test_object.fetch_last_update_information = lambda:None
        self.test_object.fetch_cache = lambda:None
        self.test_object.fetch_submodules = lambda:None
        self.test_object.fetch_metadata = lambda:None
        self.test_object.emit_signals = lambda:None
    ...
    def test_update_metadata(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for name in addon_names:
                ...
                addons.append(
                    MockAddon(name, url=f"file://localhost/{temp_dir}/{name}")
                )
            ...
            for addon in addons:
                if "HasMetadata" in addon.name:
                    self.assertIsNotNone(
                        addon.metadata,
                        f"Metadata should exist for this the test addon {addon.name}",
                    )
                else:
                    self.assertIsNone(addon.metadata)
...
""".format(
            name, f"file://localhost/{temp_dir}/{name}"
        )
